...
const NAME_COLLECTIONS = [
  {
    ...
    legacyIds: ["kidzania-koshien", "kidzania-fukuoka", "kidzania-tokyo"],
    skillName: "キッザニア図鑑調査",
    skillTags: ["勉強", "キッザニア", "職業", "図鑑"],
    category: "勉強",
    items: [
      ...
      nameEntry("観光バス"),
      nameEntry("通信会社"),
      nameEntry("電力・配電センター", ["送配電会社", "配電テクニカルセンター", "エネルギー会社"]),
      nameEntry("水道施設"),
      nameEntry("宅配会社"),
      nameEntry("警察署"),
      nameEntry("警備センター"),
      ...
      nameEntry("病院"),
      nameEntry("歯科医院"),
      nameEntry("医薬・漢方研究所", ["医薬研究所", "漢方研究所"]),
      ...
      nameEntry("ハンバーガーショップ"),
      nameEntry("パティスリーショップ"),
      nameEntry("ピザショップ"),
      nameEntry("ソーセージ工房", ["ソーセージ工場"]),
      nameEntry("冷凍食品開発センター"),
      nameEntry("ぽん酢工房", ["ポン酢工房"]),
      nameEntry("お菓子工場"),
      nameEntry("パッケージング・ラボ", ["パッケージングラボ"]),
      nameEntry("キッチンリフォームセンター"),
      nameEntry("冷蔵サポートセンター"),
      nameEntry("花屋", ["商店街/花屋"]),
      nameEntry("はんこ屋", ["商店街/はんこ屋"]),
      nameEntry("筆記具屋"),
      nameEntry("画材屋", ["商店街/画材屋"]),
      nameEntry("メガネショップ"),
      nameEntry("ビューティスタジオ"),
      nameEntry("くつ工場"),
      nameEntry("石けん工場", ["石鹸工場"]),
      nameEntry("おもちゃ工場"),
      nameEntry("ゲーム会社"),
      nameEntry("動画制作スタジオ"),
      nameEntry("ポストレコーディングスタジオ", ["ポスレコ"]),
      nameEntry("劇場", ["オープンシアター"]),
      nameEntry("楽屋", ["ステージ"]),
      nameEntry("マジックスタジオ"),
      nameEntry("ペインティングウォール"),
      nameEntry("科学研究所"),
      nameEntry("裁判所"),
      nameEntry("建設現場/サーキット", ["建設現場", "サーキット"]),
      nameEntry("建設機械開発センター"),
      nameEntry("マンション建設現場"),
      nameEntry("ホースパーク", ["ホースセンター"]),
      nameEntry("ベビージム"),
      nameEntry("エキスポセンター"),
      nameEntry("イベントスペース", ["イベントスペース1", "イベントスペース2", "イベントスペースU"]),
    ],
    titleThresholds: [
      { count: 1, title: "キッザニア職業見習い" },
      { count: 3, title: "はじめての職業体験を見守る親" },
      { count: 10, title: "キッザニア職業案内人" },
      { count: 25, title: "キッザニア職業図鑑中級者" },
      { count: 50, title: "キッザニア職業マスター" },
      { count: 68, title: "キッザニア職業図鑑コンプリート" },
    ],
  },
  {
    id: "dinosaurs",
...
function collectionState(collectionId) {
  ...
  if (collection?.legacyIds?.length && !state.nameCollections[collectionId].legacyMerged) {
    collection.legacyIds.forEach((legacyId) => {
      const legacyStore = state.nameCollections[legacyId] || {};
      Object.entries(legacyStore).forEach(([legacyName, entry]) => {
        const matchedItem = collection.items.find((item) => item.name === legacyName || (item.words || []).includes(legacyName));
        if (!matchedItem || !entry) return;
        const current = state.nameCollections[collectionId][matchedItem.name] || {};
        state.nameCollections[collectionId][matchedItem.name] = {
          count: (current.count || 0) + (entry.count || 0),
          firstLogId: current.firstLogId || entry.firstLogId,
          lastLogId: entry.lastLogId || current.lastLogId,
          logIds: Array.from(new Set([...(current.logIds || []), ...(entry.logIds || []), entry.firstLogId, entry.lastLogId].filter(Boolean))),
          updatedAt: entry.updatedAt || current.updatedAt,
        };
      });
    });
    state.nameCollections[collectionId].legacyMerged = true;
  }
...
function nameCollectionItemLogs(collection, item) {
  ...
  const collectionIds = new Set([collection.id, ...(collection.legacyIds || [])]);
  const matchedLogs = state.logs.filter((log) => {
    const matchedByCollection = (log.nameCollections || []).some(
      (group) => collectionIds.has(group.id) && (group.names || []).some((name) => item.name === name || (item.words || []).includes(name))
    );
    const matchedByText = includesAny(log.text || "", item.words || []);
    const matchedByTheme = !collection.requireThemeForItems || includesAny(log.text || "", collection.themeWords || []);
    return matchedByCollection || (matchedByText && matchedByTheme);
  });
